...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
    ...
    async ({event,step}) => {
        ...
        await step.run('check-payment-stauts', async ()=>{
...
const sendShowReminders = inngest.createFunction(
     {
        ...
        triggers: { cron: "0 */8 * * *" }   // note: your cron string has stray spaces too, see below
    },//Every 8 hours
    async ({step})=>{
        const now = new Date();
        ...
        if(reminderTaks.length === 0){
            return {
                sent:0, message:"No reminders to send."
            }
            ...
            const results = await step.run('send-all-reminders', async ()=>{
                return await Promise.allSettled(
                    reminderTasks.map(task => sendEmail({
                        ...
                        <p>
                        is scheduled for <strong>${new Date(task.showTime).toLocaleDataString('en-US', {timeZone:'Asia/Kolkata'})}</strong> at
                        <strong>${new Date(task.showTime).toLocaleTimeString('en-US', {timeZone : 'Asia/Kolkata'})}</strong>.
                        </p>
                        <p>It starts in approximately <strong> 8 hours </strong> - make sure you 're ready!
                        </p>
                        <br />
